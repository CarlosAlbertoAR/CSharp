...
public class ComandoGerarSegmentoPRemessaFebraban240Ban097 : ComandoGerarSegmentoDetalheRemessaFebraban240
{
	private const string CODIGO_CARTEIRA_PADRAO = "1";

	public ComandoGerarSegmentoPRemessaFebraban240Ban097()
	{
		Logger.Debug($"Criando comando para gerar o SEGMENTO P da remessa FEBRABAN240 do [{EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_CREDITO_NOROESTE_BRASILEIRO}]...");
	}
	...
	public override void Execute()
	{
		try
		{
			Logger.Debug($"Executando comando para gerar o SEGMENTO P da remessa FEBRABAN240 do [{EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_CREDITO_NOROESTE_BRASILEIRO}]...");
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).CodigoBancoCompensacao.Valor = EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_CREDITO_NOROESTE_BRASILEIRO.Codigo();
			...
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).UsoExclusivoFebraban1.Valor = StringUtil.ObterStringComBrancos(1);
			...
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).DvAgenciaContaCorrente.Valor = StringUtil.ObterStringComBrancos(1);
			...
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).TipoDocumento.Valor = StringUtil.ObterStringComBrancos(1);
			...
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).IdentificacaoDistribuicao.Valor = StringUtil.ObterStringComBrancos(1);
			...
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).AgenciaCobradora.Valor = StringUtil.ObterValorDeCampoMascarado(((DadoConfiguracao)base.Carteira.DadosConfiguracao[0]).Valor, 1);
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).DvAgenciaCobradora.Valor = StringUtil.ObterStringComBrancos(1);
			...
			ConfigurarJuros();
			ConfigurarDesconto();
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).ValorIOF.Valor = StringUtil.ObterStringComBrancos(15);
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).ValorAbatimento.Valor = StringUtil.ObterStringComBrancos(15);
			...
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).CodigoBaixa.Valor = StringUtil.ObterStringComBrancos(1);
			...
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).NumeroContrato.Valor = StringUtil.ObterStringComBrancos(10);
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).UsoLivreBancoEmpresa.Valor = StringUtil.ObterStringComBrancos(1);
		}
		catch (Exception ex)
		{
			...
			{
				Launcher.GetInstance().LancarExcecao(ExcecaoECobranca.ModeloBoleto.FalhaExecutandoComandoGerarSegmentoPRemessaFebraban240(EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_CREDITO_NOROESTE_BRASILEIRO, ex), new StackTrace().GetFrame(1).GetMethod());
			}
		}
	}
	...
	private void ConfigurarDesconto()
	{
		...
		(base.Segmento as SegmentoDetalhePFebraban240Padrao).DataDesconto1.Valor = StringUtil.ObterStringComBrancos(8);
		(base.Segmento as SegmentoDetalhePFebraban240Padrao).Desconto1.Valor = StringUtil.ObterStringComBrancos(15);
	...
	private void ConfigurarJuros()
	{
		...
		(base.Segmento as SegmentoDetalhePFebraban240Padrao).DataJuros.Valor = StringUtil.ObterStringComBrancos(8);
		(base.Segmento as SegmentoDetalhePFebraban240Padrao).JurosMora.Valor = (string)base.DoubleConverter.Convert(0, typeof(string), "F4", CultureInfo.CurrentCulture);
		if (!string.IsNullOrEmpty(base.Documento.CodigoJuros) || !string.IsNullOrEmpty(base.Carteira.ConfiguracaoDocumentoCobranca.CodigoJuros))
		{
			...
			case EnumCodigoJuros.TAXA_MENSAL:
				num = ((base.Documento.ValorJurosDiasAtraso > 0.0) ? (base.Documento.ValorJurosDiasAtraso / base.Documento.ValorDocumento * 30.0 * 100.0) : ((base.Documento.PercentualJurosDiaAtraso > 0.0) ? (base.Documento.PercentualJurosDiaAtraso * 30.0) : 0.0));
				break;
			}
			if (num > 0.0)
			{
