...
public class ComandoGerarSegmentoQRemessaFebraban240Ban104 : ComandoGerarSegmentoDetalheRemessaFebraban240
{
	public ComandoGerarSegmentoQRemessaFebraban240Ban104()
	{
		Logger.Debug($"Criando comando para gerar o SEGMENTO Q da remessa FEBRABAN240 do [{EnumCodigoBanco.CAIXA_ECONOMICA_FEDERAL}]...");
	}
	...
	public override void Execute()
	{
		try
		{
			Logger.Debug($"Executando o comando para gerar o SEGMENTO Q da remessa FEBRABAN240 do [{EnumCodigoBanco.CAIXA_ECONOMICA_FEDERAL}]...");
			(base.Segmento as SegmentoDetalheQFebraban240Padrao).CodigoBancoCompensacao.Valor = EnumCodigoBanco.CAIXA_ECONOMICA_FEDERAL.Codigo();
			...
			{
				(base.Segmento as SegmentoDetalheQFebraban240Padrao).TipoInscricaoSacado.Valor = EnumTipoInscricao.ISENTO_OU_NAO_INFORMADO.Codigo();
			}
			if (!string.IsNullOrEmpty(base.Documento.CnpjPagador))
			{
			...
			{
				(base.Segmento as SegmentoDetalheQFebraban240Padrao).NumeroInscricaoSacado.Valor = "0";
			}
			...
			{
				(base.Segmento as SegmentoDetalheQFebraban240Padrao).TipoInscricaoSacadorAvalista.Valor = EnumTipoInscricao.ISENTO_OU_NAO_INFORMADO.Codigo();
			}
			if (!string.IsNullOrEmpty(base.Documento.CnpjAvalista))
			{
			...
			{
				(base.Segmento as SegmentoDetalheQFebraban240Padrao).NumeroInscricaoSacadorAvalista.Valor = "0";
			}
			(base.Segmento as SegmentoDetalheQFebraban240Padrao).NomeSacadorAvalista.Valor = base.Documento.SacadorAvalista;
			(base.Segmento as SegmentoDetalheQFebraban240Padrao).CodigoBancoCorrespondente.Valor = StringUtil.ObterStringComBrancos(3);
		...
		catch (Exception ex)
		{
			...
			{
				Launcher.GetInstance().LancarExcecao(ExcecaoECobranca.ModeloBoleto.FalhaExecutandoComandoGerarSegmentoQRemessaFebraban240(EnumCodigoBanco.CAIXA_ECONOMICA_FEDERAL, ex), new StackTrace().GetFrame(1).GetMethod());
			}
		}
	}
}
