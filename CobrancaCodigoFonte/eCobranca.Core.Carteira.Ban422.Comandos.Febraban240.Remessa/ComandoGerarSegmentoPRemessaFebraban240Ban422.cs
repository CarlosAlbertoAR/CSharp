...
public class ComandoGerarSegmentoPRemessaFebraban240Ban422 : ComandoGerarSegmentoDetalheRemessaFebraban240
{
	public ComandoGerarSegmentoPRemessaFebraban240Ban422()
	{
		Logger.Debug($"Criando comando para gerar o SEGMENTO P da remessa FEBRABAN240 do [{EnumCodigoBanco.BANCO_SAFRA}]...");
	}
	...
	public override void Execute()
	{
		try
		{
			Logger.Debug($"Executando comando para gerar o SEGMENTO P da remessa FEBRABAN240 do [{EnumCodigoBanco.BANCO_SAFRA}]...");
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).CodigoBancoCompensacao.Valor = EnumCodigoBanco.BANCO_SAFRA.Codigo();
			...
			{
				(base.Segmento as SegmentoDetalhePFebraban240Padrao).IdentificacaoTituloNoBanco.Valor = StringUtil.RemoverMascara(base.Documento.NossoNumero) + StringUtil.ObterStringComBrancos(11);
			}
			if (string.IsNullOrEmpty(base.Documento.ModalidadeCobranca))
			{
				(base.Segmento as SegmentoDetalhePFebraban240Padrao).CodigoCarteira.Valor = ((DadoConfiguracao)base.Carteira.DadosConfiguracao[2]).Valor.ToEnum<EnumModalidadeCarteiraCobranca>().Codigo();
			}
			else
			{
			...
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).AgenciaCobradora.Valor = StringUtil.RemoverMascara(((DadoConfiguracao)base.Carteira.DadosConfiguracao[0]).Valor);
		...
		catch (Exception ex)
		{
			...
			{
				Launcher.GetInstance().LancarExcecao(ExcecaoECobranca.ModeloBoleto.FalhaExecutandoComandoGerarSegmentoPRemessaFebraban240(EnumCodigoBanco.BANCO_SAFRA, ex), new StackTrace().GetFrame(1).GetMethod());
			}
		}
	}
	...
	private void ConfigurarJuros()
	{
		...
		(base.Segmento as SegmentoDetalhePFebraban240Padrao).DataJuros.Valor = (string)base.DateConverter.Convert(base.Documento.DataVencimento.Value.AddDays(1.0), typeof(string), EnumTipoData.DDMMAAAA, CultureInfo.CurrentCulture);
