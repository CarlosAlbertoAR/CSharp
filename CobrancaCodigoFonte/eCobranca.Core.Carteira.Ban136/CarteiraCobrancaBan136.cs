...
using eCobranca.Core.Carteira.Ban136.Comandos.Cnab400.Remessa;
using eCobranca.Core.Carteira.Ban136.Comandos.Cnab400.Retorno;
...
[Serializable]
public abstract class CarteiraCobrancaBan136 : CarteiraCobranca, ISerializable
{
	public CarteiraCobrancaBan136(ConfiguracaoCarteiraCobranca configuracaoCarteiraCobranca)
		: base(configuracaoCarteiraCobranca)
	{
	}

	public CarteiraCobrancaBan136(SerializationInfo serobj, StreamingContext scontext)
	...
	protected override ConfiguracaoBoleto CarregarConfiguracaoBoleto()
	{
		...
		try
		{
			Logger.Debug($"Criando configuração inicial de boleto do [{EnumCodigoBanco.UNICRED}]...");
			...
			result = new ConfiguracaoBoleto(GetGuid(), EnumCodigoBanco.UNICRED.ToString(), base.ModoUso, this, "Central de Atendimento Unicred: (65) 3314-4700 Ouvidoria Unicred: 0800 940 0602 (segunda à sexta-feira, horário de atendimento: 09:00 às 18:00)", "136-8", GetCodigoCarteiraImpressao());
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override ConfiguracaoArquivoRemessa CarregarConfiguracaoArquivoRemessa()
	{
		...
		try
		{
			Logger.Debug($"Criando configuração inicial de remessa de carteiras do banco do [{EnumCodigoBanco.UNICRED}]");
			configuracaoArquivoRemessa = new ConfiguracaoArquivoRemessa(GetGuid(), EnumCodigoBanco.UNICRED.ToString(), base.ModoUso, this, 0, base.ConfiguracaoCarteiraCobranca.CobrancaRegistrada);
	...
	{
		return (ArquivoCobrancaRemessaFebraban240)FactoryArquivoCobranca.CriarArquivoRemessa(GetGuid(), EnumLayoutArquivoRemessa.FEBRABAN240, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.UNICRED);
	}

	protected override ArquivoCobrancaRemessaCnab400 CarregarArquivoRemessaCnab400()
	{
		return (ArquivoCobrancaRemessaCnab400Ban136)FactoryArquivoCobranca.CriarArquivoRemessa(GetGuid(), EnumLayoutArquivoRemessa.CNAB400, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.UNICRED);
	}
	...
	{
		return new ComandoGerarArquivoRemessaCnab400Ban136();
	}
	...
	protected override ConfiguracaoArquivoRetorno CarregarConfiguracaoArquivoRetorno()
	{
		...
		try
		{
			Logger.Debug($"Criando configuração inicial de retorno de carteiras do banco do [{EnumCodigoBanco.UNICRED}]");
			result = new ConfiguracaoArquivoRetorno(GetGuid(), EnumCodigoBanco.UNICRED.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	{
		return (ArquivoCobrancaRetornoCnab400Ban136)FactoryArquivoCobranca.CriarArquivoRetorno(GetGuid(), EnumLayoutArquivoRetorno.CNAB400, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.UNICRED);
	}
	...
	{
		return new ComandoCarregarArquivoRetornoCnab400Ban136();
	}
	...
	protected override ConfiguracaoImpressao CarregarConfiguracaoImpressao()
	{
		...
		try
		{
			Logger.Debug($"Carregando configuração de impressão de documentos de cobrança do [{EnumCodigoBanco.UNICRED}]...");
			result = new ConfiguracaoImpressao(GetGuid(), EnumCodigoBanco.UNICRED.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override ConfiguracaoEmail CarregarConfiguracaoEmail()
	{
		...
		{
			Logger.Debug($"Carregando configuração de impressão de documentos de cobrança do [{EnumCodigoBanco.UNICRED}]...");
			result = new ConfiguracaoEmail(GetGuid(), EnumCodigoBanco.UNICRED.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override ConfiguracaoDocumentoCobranca CarregarConfiguracaoDocumentoCobranca()
	{
		...
		try
		{
			Logger.Debug($"Carregando configuração de documentos de cobrança do [{EnumCodigoBanco.UNICRED}]...");
			result = new ConfiguracaoDocumentoCobranca(GetGuid(), EnumCodigoBanco.UNICRED.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override string GerarArquivoRemessaCNAB400(ArrayList dcList)
	{
		...
		try
		{
			Logger.Debug($"Gerando arquivo de remessa CNAB400 para o [{EnumCodigoBanco.UNICRED}]...");
			remessaCnab400.Preparar(base.ConfiguracaoArquivoRemessa);
			remessaCnab400.CriarLote(dcList);
			(comandoGerarRemessaCnab400 as ComandoGerarArquivoRemessaCnab400Ban136).Preparar(this, remessaCnab400);
	...
	protected override ArrayList CarregarArquivoRetornoCnab400(string nomeArquivoRetorno)
	{
		...
		try
		{
			Logger.Debug($"Processando arquivo de retorno CNAB400 do [{EnumCodigoBanco.UNICRED}]...");
			retornoCnab400.Preparar(base.ConfiguracaoArquivoRetorno.DiretorioArquivoRetorno, nomeArquivoRetorno);
			(comandoProcessarRetornoCnab400 as ComandoCarregarArquivoRetornoCnab400Ban136).Preparar(this, retornoCnab400);
	...
	{
		return new ProvedorDadosModeloBoletoBan136();
	}
	...
	protected override void CarregarDadosConfiguracao()
	{
		...
		base.DadosConfiguracao.Add(value);
		value = new DadoConfiguracao("CodigoBeneficiario", "Código do beneficiário na UNICRED", "Código do beneficiário", "00000000");
		base.DadosConfiguracao.Add(value);
		value = new DadoConfiguracao("Modalidade", "Modalidade de cobranca: Utilize um dos valores da lista ConfiguracaoDocumentoCobranca.ModalidadesCobranca", "Modalidade de cobrança", "0", EnumUtil.GetEnumByAttributeToItemLista<EnumModalidadeCarteiraCobranca, AtributoChave>(new AtributoChave(EnumCodigoBanco.UNICRED)));
		base.DadosConfiguracao.Add(value);
		value = new DadoConfiguracao("InicioNossoNumero", "Sequência inicial para geração do nosso número", "Início nosso número", "0000000000");
		value.Valor = "0000000001";
		base.DadosConfiguracao.Add(value);
		value = new DadoConfiguracao("FimNossoNumero", "Final da sequência para geração do nosso número", "Fim do nosso número", "0000000000");
		value.Valor = "9999999999";
		base.DadosConfiguracao.Add(value);
		value = new DadoConfiguracao("NossoNumeroAtual", "Sequencia atual para geração do nosso número", "Nosso número atual", "0000000000");
	...
	{
		return new ParametrosAdicionaisDisponiveisBan136();
	}

	protected override string GerarNomeArquivoRemessa()
	{
		ConverterDateToString converterDateToString = new ConverterDateToString(EnumTipoData.DDMMAA);
		...
		return "CNAB400_UNICRED_" + StringUtil.ObterValorDeCampoMascarado(((DadoConfiguracao)base.DadosConfiguracao[0]).Valor, 1) + "_" + (string)converterDateToString.Convert(DateTime.Today, typeof(string), EnumTipoData.DDMMAA, CultureInfo.CurrentCulture) + "_" + text + ".REM";
	}
	...
	protected override string GetCampoLivre(DocumentoCobranca dc)
	{
		...
		try
		{
			...
			result = StringUtil.ObterValorDeCampoMascarado(((DadoConfiguracao)base.DadosConfiguracao[0]).Valor, 1) + StringUtil.RemoverMascara(((DadoConfiguracao)base.DadosConfiguracao[1]).Valor) + StringUtil.RemoverMascara(dc.NossoNumero);
			return result;
		}
		catch (Exception ex)
		{
