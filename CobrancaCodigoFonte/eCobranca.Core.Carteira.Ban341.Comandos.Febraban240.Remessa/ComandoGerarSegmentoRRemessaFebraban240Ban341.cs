...
public class ComandoGerarSegmentoRRemessaFebraban240Ban341 : ComandoGerarSegmentoDetalheRemessaFebraban240
{
	public ComandoGerarSegmentoRRemessaFebraban240Ban341()
	{
		Logger.Debug($"Criando comando para gerar o SEGMENTO R da remessa FEBRABAN240 do [{EnumCodigoBanco.BANCO_ITAU}]...");
	}
	...
	public override void Execute()
	{
		try
		{
			Logger.Debug($"Executando comando para gerar o SEGMENTO R da remessa FEBRABAN240 do [{EnumCodigoBanco.BANCO_ITAU}]...");
			(base.Segmento as SegmentoDetalheRFebraban240Padrao).CodigoBancoCompensacao.Valor = EnumCodigoBanco.BANCO_ITAU.Codigo();
			...
			{
				(base.Segmento as SegmentoDetalheRFebraban240Padrao).DataDesconto2.Valor = valorParametroAdicional;
			}
			...
			{
				(base.Segmento as SegmentoDetalheRFebraban240Padrao).Desconto2.Valor = valorParametroAdicional2;
			}
			...
			{
				(base.Segmento as SegmentoDetalheRFebraban240Padrao).DataDesconto3.Valor = valorParametroAdicional3;
			}
			...
			{
				(base.Segmento as SegmentoDetalheRFebraban240Padrao).Desconto3.Valor = valorParametroAdicional4;
			}
			ConfigurarMulta();
		...
		catch (Exception ex)
		{
			...
			{
				Launcher.GetInstance().LancarExcecao(ExcecaoECobranca.ModeloBoleto.FalhaExecutandoComandoGerarSegmentoRRemessaFebraban240(EnumCodigoBanco.BANCO_ITAU, ex), new StackTrace().GetFrame(1).GetMethod());
			}
		}
	}
