...
using eCobranca.Core.Carteira.Ban099.Comandos.Cnab400.Remessa;
using eCobranca.Core.Carteira.Ban099.Comandos.Cnab400.Retorno;
...
[Serializable]
public abstract class CarteiraCobrancaBan099 : CarteiraCobranca, ISerializable
{
	public CarteiraCobrancaBan099(ConfiguracaoCarteiraCobranca configuracaoCarteiraCobranca)
		: base(configuracaoCarteiraCobranca)
	{
	}

	public CarteiraCobrancaBan099(SerializationInfo serobj, StreamingContext scontext)
	...
	protected override ConfiguracaoBoleto CarregarConfiguracaoBoleto()
	{
		...
		try
		{
			Logger.Debug($"Criando configuração inicial de boleto do [{EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED}]...");
			...
			result = new ConfiguracaoBoleto(GetGuid(), EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED.ToString(), base.ModoUso, this, "SEDE LONDRINA-PR AV. RIO DE JANEIRO, 1.758 - CENTRO | FONE: (43) 3294-1900 Ouvidoria Uniprime funciona de segunda à sexta-feira das 08:30 às 12:00h e 13:30 às 17:00h pelo telefone 0800 645 3737", "099-X", GetCodigoCarteiraImpressao());
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override ConfiguracaoArquivoRemessa CarregarConfiguracaoArquivoRemessa()
	{
		...
		try
		{
			Logger.Debug($"Criando configuração inicial de remessa de carteiras do banco do [{EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED}]");
			configuracaoArquivoRemessa = new ConfiguracaoArquivoRemessa(GetGuid(), EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED.ToString(), base.ModoUso, this, 0, base.ConfiguracaoCarteiraCobranca.CobrancaRegistrada);
	...
	{
		return (ArquivoCobrancaRemessaCnab400Ban099)FactoryArquivoCobranca.CriarArquivoRemessa(GetGuid(), EnumLayoutArquivoRemessa.CNAB400, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED);
	}
	...
	{
		return new ComandoGerarArquivoRemessaCnab400Ban099();
	}
	...
	protected override ConfiguracaoArquivoRetorno CarregarConfiguracaoArquivoRetorno()
	{
		...
		try
		{
			Logger.Debug($"Criando configuração inicial de retorno de carteiras do banco do [{EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED}]");
			result = new ConfiguracaoArquivoRetorno(GetGuid(), EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	{
		return (ArquivoCobrancaRetornoCnab400Ban099)FactoryArquivoCobranca.CriarArquivoRetorno(GetGuid(), EnumLayoutArquivoRetorno.CNAB400, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED);
	}
	...
	{
		return new ComandoCarregarArquivoRetornoCnab400Ban099();
	}
	...
	protected override ConfiguracaoImpressao CarregarConfiguracaoImpressao()
	{
		...
		try
		{
			Logger.Debug($"Carregando configuração de impressão de documentos de cobrança do [{EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED}]...");
			result = new ConfiguracaoImpressao(GetGuid(), EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override ConfiguracaoEmail CarregarConfiguracaoEmail()
	{
		...
		{
			Logger.Debug($"Carregando configuração de impressão de documentos de cobrança do [{EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED}]...");
			result = new ConfiguracaoEmail(GetGuid(), EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override ConfiguracaoDocumentoCobranca CarregarConfiguracaoDocumentoCobranca()
	{
		...
		try
		{
			Logger.Debug($"Carregando configuração de documentos de cobrança do [{EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED}]...");
			result = new ConfiguracaoDocumentoCobranca(GetGuid(), EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override string GerarArquivoRemessaCNAB400(ArrayList dcList)
	{
		...
		try
		{
			...
			{
				Logger.Debug($"Gerando arquivo de remessa CNAB400 para o [{EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED}]...");
				remessaCnab400.Preparar(base.ConfiguracaoArquivoRemessa);
				remessaCnab400.CriarLote(dcList);
				(comandoGerarRemessaCnab400 as ComandoGerarArquivoRemessaCnab400Ban099).Preparar(this, remessaCnab400);
	...
	protected override ArrayList CarregarArquivoRetornoCnab400(string nomeArquivoRetorno)
	{
		...
		try
		{
			Logger.Debug($"Processando arquivo de retorno CNAB400 do [{EnumCodigoBanco.COOPERATIVA_CENTRAL_DE_ECONOMIA_E_CREDITO_MUTUO_DAS_UNICRED}]...");
			retornoCnab400.Preparar(base.ConfiguracaoArquivoRetorno.DiretorioArquivoRetorno, nomeArquivoRetorno);
			(comandoProcessarRetornoCnab400 as ComandoCarregarArquivoRetornoCnab400Ban099).Preparar(this, retornoCnab400);
	...
	protected override void CarregarDadosConfiguracao()
	{
		...
		value = new DadoConfiguracao("NumeroContaCorrente", "Conta do cooperado na Uniprime", "Conta corrente", "0000000-A");
		base.DadosConfiguracao.Add(value);
	...
	protected override string GetNossoNumeroSemDV(DocumentoCobranca dc)
	{
		...
		try
		{
			...
			result = ((DadoConfiguracao)base.DadosConfiguracao[2]).Valor.ToEnum<EnumModalidadeCarteiraCobranca>().Codigo() + StringUtil.ObterValorDeCampoMascarado(((DadoConfiguracao)base.DadosConfiguracao[0]).Valor, 1) + valor;
			return result;
		}
		catch (Exception ex)
		{
	...
	{
		return new ParametrosAdicionaisDisponiveisBan099();
	}
	...
	{
		return new ProvedorDadosModeloBoletoBan099();
	}
}
