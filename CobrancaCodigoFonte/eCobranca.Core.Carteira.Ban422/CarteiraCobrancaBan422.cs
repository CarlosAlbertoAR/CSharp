...
using eCobranca.Core.Carteira.Ban422.Comandos.Cnab400.Remessa;
using eCobranca.Core.Carteira.Ban422.Comandos.Cnab400.Retorno;
using eCobranca.Core.Carteira.Ban422.Comandos.Febraban240.Remessa;
using eCobranca.Core.Carteira.Ban422.Comandos.Febraban240.Retorno;
...
[Serializable]
public abstract class CarteiraCobrancaBan422 : CarteiraCobranca, ISerializable
{
	public CarteiraCobrancaBan422(ConfiguracaoCarteiraCobranca configuracaoCarteiraCobranca)
		: base(configuracaoCarteiraCobranca)
	{
	}

	public CarteiraCobrancaBan422(SerializationInfo serobj, StreamingContext scontext)
	...
	protected override ConfiguracaoBoleto CarregarConfiguracaoBoleto()
	{
		...
		try
		{
			Logger.Debug($"Criando configuração inicial de boleto do [{EnumCodigoBanco.BANCO_SAFRA}]...");
			...
			result = new ConfiguracaoBoleto(GetGuid(), EnumCodigoBanco.BANCO_SAFRA.ToString(), base.ModoUso, this, "SAC Safra: 0800 772 5755 (gratuito, qualquer localidade). Ouvidoria Safra: 0800 770 1236 (segunda à sexta-feira, das 9h às 18h)", "422-7", GetCodigoCarteiraImpressao());
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override ConfiguracaoArquivoRemessa CarregarConfiguracaoArquivoRemessa()
	{
		...
		try
		{
			Logger.Debug($"Criando configuração inicial de remessa de carteiras do banco do [{EnumCodigoBanco.BANCO_SAFRA}]");
			configuracaoArquivoRemessa = new ConfiguracaoArquivoRemessa(GetGuid(), EnumCodigoBanco.BANCO_SAFRA.ToString(), base.ModoUso, this, 0, base.ConfiguracaoCarteiraCobranca.CobrancaRegistrada);
	...
	{
		return (ArquivoCobrancaRemessaFebraban240)FactoryArquivoCobranca.CriarArquivoRemessa(GetGuid(), EnumLayoutArquivoRemessa.FEBRABAN240, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.BANCO_SAFRA);
	}

	protected override ArquivoCobrancaRemessaCnab400 CarregarArquivoRemessaCnab400()
	{
		return (ArquivoCobrancaRemessaCnab400Ban422)FactoryArquivoCobranca.CriarArquivoRemessa(GetGuid(), EnumLayoutArquivoRemessa.CNAB400, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.BANCO_SAFRA);
	}

	protected override MacroCommand CarregarComandoGerarRemessaFebraban240()
	{
		return new ComandoGerarArquivoRemessaFebraban240Ban422(this);
	}

	protected override MacroCommand CarregarComandoGerarRemessaCnab400()
	{
		return new ComandoGerarArquivoRemessaCnab400Ban422();
	}
	...
	protected override ConfiguracaoArquivoRetorno CarregarConfiguracaoArquivoRetorno()
	{
		...
		try
		{
			Logger.Debug($"Criando configuração inicial de retorno de carteiras do banco do [{EnumCodigoBanco.BANCO_SAFRA}]");
			result = new ConfiguracaoArquivoRetorno(GetGuid(), EnumCodigoBanco.BANCO_SAFRA.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	{
		return (ArquivoCobrancaRetornoFebraban240)FactoryArquivoCobranca.CriarArquivoRetorno(GetGuid(), EnumLayoutArquivoRetorno.FEBRABAN240, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.BANCO_SAFRA);
	}

	protected override ArquivoCobrancaRetornoCnab400 CarregarArquivoRetornoCnab400()
	{
		return (ArquivoCobrancaRetornoCnab400Ban422)FactoryArquivoCobranca.CriarArquivoRetorno(GetGuid(), EnumLayoutArquivoRetorno.CNAB400, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.BANCO_SAFRA);
	}
	...
	{
		return new ComandoCarregarArquivoRetornoFebraban240Ban422(this);
	}

	protected override MacroCommand CarregarComandoProcessarRetornoCnab400()
	{
		return new ComandoCarregarArquivoRetornoCnab400Ban422();
	}
	...
	protected override ConfiguracaoImpressao CarregarConfiguracaoImpressao()
	{
		...
		try
		{
			Logger.Debug($"Carregando configuração de impressão de documentos de cobrança do [{EnumCodigoBanco.BANCO_SAFRA}]...");
			result = new ConfiguracaoImpressao(GetGuid(), EnumCodigoBanco.BANCO_SAFRA.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override ConfiguracaoEmail CarregarConfiguracaoEmail()
	{
		...
		try
		{
			Logger.Debug($"Carregando configuração de email do cobrança do [{EnumCodigoBanco.BANCO_SAFRA}]...");
			result = new ConfiguracaoEmail(GetGuid(), EnumCodigoBanco.BANCO_SAFRA.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override ConfiguracaoDocumentoCobranca CarregarConfiguracaoDocumentoCobranca()
	{
		...
		try
		{
			Logger.Debug($"Carregando configuração de documentos de cobrança do [{EnumCodigoBanco.BANCO_SAFRA}]...");
			result = new ConfiguracaoDocumentoCobranca(GetGuid(), EnumCodigoBanco.BANCO_SAFRA.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override string GerarArquivoRemessaFebraban240(ArrayList dcList)
	{
		...
		try
		{
			...
			{
				Logger.Debug($"Gerando arquivo de remessa FEBRABAN240 para o [{EnumCodigoBanco.BANCO_SAFRA}]...");
				remessaFebraban240.Preparar(this);
				remessaFebraban240.NovoLote(dcList);
				(comandoGerarRemessaFebraban240 as ComandoGerarArquivoRemessaFebraban240Ban422).Preparar(this, remessaFebraban240);
	...
	protected override string GerarArquivoRemessaCNAB400(ArrayList dcList)
	{
		...
		try
		{
			if (base.ConfiguracaoCarteiraCobranca.CobrancaRegistrada)
			{
				Logger.Debug($"Gerando arquivo de remessa CNAB400 para o [{EnumCodigoBanco.BANCO_SAFRA}]...");
				remessaCnab400.Preparar(base.ConfiguracaoArquivoRemessa);
				remessaCnab400.CriarLote(dcList);
				(comandoGerarRemessaCnab400 as ComandoGerarArquivoRemessaCnab400Ban422).Preparar(this, remessaCnab400);
	...
	protected override ArrayList CarregarArquivoRetornoFebraban240(string nomeArquivoRetorno)
	{
		...
		try
		{
			Logger.Debug($"Processando arquivo de retorno FEBRABAN240 do [{EnumCodigoBanco.BANCO_SAFRA}]...)");
			retornoFebraban240.Preparar(this, base.ConfiguracaoArquivoRetorno.DiretorioArquivoRetorno, nomeArquivoRetorno);
			(comandoProcessarRetornoFebraban240 as ComandoCarregarArquivoRetornoFebraban240Ban422).Preparar(this, retornoFebraban240);
	...
	protected override ArrayList CarregarArquivoRetornoCnab400(string nomeArquivoRetorno)
	{
		...
		try
		{
			Logger.Debug($"Processando arquivo de retorno CNAB400 do [{EnumCodigoBanco.BANCO_SAFRA}]...");
			retornoCnab400.Preparar(base.ConfiguracaoArquivoRetorno.DiretorioArquivoRetorno, nomeArquivoRetorno);
			(comandoProcessarRetornoCnab400 as ComandoCarregarArquivoRetornoCnab400Ban422).Preparar(this, retornoCnab400);
	...
	{
		return new ProvedorDadosModeloBoletoBan422();
	}
	...
	protected override string GetCampoLivre(DocumentoCobranca dc)
	{
		...
		try
		{
			...
			result = "7" + StringUtil.RemoverMascara(((DadoConfiguracao)base.DadosConfiguracao[0]).Valor) + StringUtil.RemoverMascara(((DadoConfiguracao)base.DadosConfiguracao[1]).Valor) + StringUtil.RemoverMascara(dc.NossoNumero) + "2";
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override string CalcularDVNossoNumero(string nossoNumeroSemDV)
	{
		...
		try
		{
			...
			int num7 = num4 % 11;
			text = ((num7 > 1 && num7 <= 10) ? Convert.ToString(11 - num7) : ((num7 != 1) ? "1" : "0"));
		}
		catch (Exception ex)
		{
			if (ex is ExcecaoECobranca)
	...
	protected override void CarregarDadosConfiguracao()
	{
		...
		DadoConfiguracao value = new DadoConfiguracao("CodigoAgencia", "Código da agência bancária", "Agência", "00000");
		base.DadosConfiguracao.Add(value);
		value = new DadoConfiguracao("NumeroContaCorrente", "Número da conta corrente", "Conta corrente", "00000000-0");
		base.DadosConfiguracao.Add(value);
		value = new DadoConfiguracao("Modalidade", "Modalidade de cobranca: Utilize um dos valores da lista ConfiguracaoDocumentoCobranca.ModalidadesCobranca", "Modalidade de cobrança", "0", EnumUtil.GetEnumByAttributeToItemLista<EnumModalidadeCarteiraCobranca, AtributoChave>(new AtributoChave(EnumCodigoBanco.BANCO_SAFRA)));
		base.DadosConfiguracao.Add(value);
	...
	{
		return new ParametrosAdicionaisDisponiveisBan422();
	}
