...
public class ComandoGerarSegmentoPRemessaFebraban240Ban037 : ComandoGerarSegmentoDetalheRemessaFebraban240
{
	public ComandoGerarSegmentoPRemessaFebraban240Ban037()
	{
		Logger.Debug($"Criando comando para gerar o SEGMENTO P da remessa FEBRABAN240 do [{EnumCodigoBanco.BANCO_DO_ESTADO_DO_PARA}]...");
	}
	...
	public override void Execute()
	{
		try
		{
			Logger.Debug($"Executando comando para gerar o SEGMENTO P da remessa FEBRABAN240 do [{EnumCodigoBanco.BANCO_DO_ESTADO_DO_PARA}]...");
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).CodigoBancoCompensacao.Valor = EnumCodigoBanco.BANCO_DO_ESTADO_DO_PARA.Codigo();
			...
			(base.Segmento as SegmentoDetalhePFebraban240Padrao).DvContaCorrente.Valor = StringUtil.ObterValorDeCampoMascarado(((DadoConfiguracao)base.Carteira.DadosConfiguracao[1]).Valor, 2);
			...
			if (base.Carteira.DadosConfiguracao[3] != null && !string.IsNullOrEmpty(((DadoConfiguracao)base.Carteira.DadosConfiguracao[3]).Valor))
			{
				(base.Segmento as SegmentoDetalhePFebraban240Padrao).CodigoCarteira.Valor = ((EnumModalidadeCarteiraCobranca)Enum.Parse(typeof(EnumModalidadeCarteiraCobranca), ((DadoConfiguracao)base.Carteira.DadosConfiguracao[3]).Valor)).Codigo();
			}
			else
			{
				(base.Segmento as SegmentoDetalhePFebraban240Padrao).CodigoCarteira.Valor = EnumModalidadeCarteiraCobranca.SIMPLES.Codigo();
			}
			...
			else if (base.Carteira.ConfiguracaoDocumentoCobranca.DiasBaixa > 0)
			{
		...
		catch (Exception ex)
		{
			...
			{
				Launcher.GetInstance().LancarExcecao(ExcecaoECobranca.ModeloBoleto.FalhaExecutandoComandoGerarSegmentoPRemessaFebraban240(EnumCodigoBanco.BANCO_DO_ESTADO_DO_PARA, ex), new StackTrace().GetFrame(1).GetMethod());
			}
		}
	}
