...
public class ArquivoCobrancaRetornoFebraban240Ban037 : ArquivoCobrancaRetornoFebraban240
{
	public ArquivoCobrancaRetornoFebraban240Ban037(Guid guid, EnumTipoCarteiraCobranca tipoCarteiraCobranca, EnumCodigoBanco banco)
					...
					foreach (LoteCobrancaRetornoFebraban240 lote in lotes)
					{
						foreach (SegmentoDetalheFebraban240 item in lote.SegmentosDetalhe)
						{
							if (item.CodigoSegmentoRegistroDetalhe.Valor == EnumTipoSegmentoDetalheFebraban240.SEGMENTO_T.Codigo())
							{
								...
								ocorrenciaCobranca.CodigoAgencia = (headerArquivo as HeaderArquivoFebraban240Padrao).CodigoAgencia.Valor.Trim();
								...
								if (ocorrenciaCobranca.Nossonumero.Length != base.Carteira.GetMascaraNossoNumero().Length)
								{
									ocorrenciaCobranca.Nossonumero = StringUtil.ZerosLeft(ocorrenciaCobranca.Nossonumero, base.Carteira.GetMascaraNossoNumero().Length);
								}
								...
								ocorrenciaCobranca.CodigoMovimentoRetorno = StringUtil.ZerosLeft((item as SegmentoDetalheTFebraban240Padrao).CodigoMovimento.Valor.Trim(), 2);
								...
								ocorrenciaCobranca.ValorNominal = (double)doubleToStringConverter.ConvertBack((item as SegmentoDetalheTFebraban240Padrao).ValorNominalTitulo.Valor.Trim(), typeof(decimal), null, CultureInfo.CurrentCulture);
								ocorrenciaCobranca.ValorTarifa = (double)doubleToStringConverter.ConvertBack((item as SegmentoDetalheTFebraban240Padrao).ValorTarifaCustas.Valor.Trim(), typeof(decimal), null, CultureInfo.CurrentCulture);
							...
							if (item.CodigoSegmentoRegistroDetalhe.Valor == EnumTipoSegmentoDetalheFebraban240.SEGMENTO_U.Codigo())
							{
								...
								ocorrenciaCobranca.ValorAcrescimos = (double)doubleToStringConverter.ConvertBack((item as SegmentoDetalheUFebraban240Padrao).ValorAcrescimos.Valor.Trim(), typeof(decimal), null, CultureInfo.CurrentCulture);
								ocorrenciaCobranca.ValorDesconto = (double)doubleToStringConverter.ConvertBack((item as SegmentoDetalheUFebraban240Padrao).ValorDesconto.Valor.Trim(), typeof(decimal), null, CultureInfo.CurrentCulture);
								ocorrenciaCobranca.ValorAbatimento = (double)doubleToStringConverter.ConvertBack((item as SegmentoDetalheUFebraban240Padrao).ValorAbatimento.Valor.Trim(), typeof(decimal), null, CultureInfo.CurrentCulture);
								ocorrenciaCobranca.ValorIOF = (double)doubleToStringConverter.ConvertBack((item as SegmentoDetalheUFebraban240Padrao).ValorIOF.Valor.Trim(), typeof(decimal), null, CultureInfo.CurrentCulture);
								ocorrenciaCobranca.ValorOutrasDespesas = (double)doubleToStringConverter.ConvertBack((item as SegmentoDetalheUFebraban240Padrao).ValorOutrasDespesas.Valor.Trim(), typeof(decimal), null, CultureInfo.CurrentCulture);
								ocorrenciaCobranca.ValorOutrosCreditos = (double)doubleToStringConverter.ConvertBack((item as SegmentoDetalheUFebraban240Padrao).ValorOutrosCreditos.Valor.Trim(), typeof(decimal), null, CultureInfo.CurrentCulture);
								ocorrenciaCobranca.ValorOcorrenciaSacado = (double)doubleToStringConverter.ConvertBack((item as SegmentoDetalheUFebraban240Padrao).ValorOcorrenciaSacado.Valor.Trim(), typeof(decimal), null, CultureInfo.CurrentCulture);
								ocorrenciaCobranca.ValorPago = (double)doubleToStringConverter.ConvertBack((item as SegmentoDetalheUFebraban240Padrao).ValorPago.Valor.Trim(), typeof(decimal), null, CultureInfo.CurrentCulture);
								ocorrenciaCobranca.ValorLiquido = (double)doubleToStringConverter.ConvertBack((item as SegmentoDetalheUFebraban240Padrao).ValorLiquido.Valor.Trim(), typeof(decimal), null, CultureInfo.CurrentCulture);
