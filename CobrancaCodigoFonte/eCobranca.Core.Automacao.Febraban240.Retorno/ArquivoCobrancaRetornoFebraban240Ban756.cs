...
public class ArquivoCobrancaRetornoFebraban240Ban756 : ArquivoCobrancaRetornoFebraban240
{
	public ArquivoCobrancaRetornoFebraban240Ban756(Guid guid, EnumTipoCarteiraCobranca tipoCarteiraCobranca, EnumCodigoBanco banco)
					...
					foreach (LoteCobrancaRetornoFebraban240 lote in lotes)
					{
						foreach (SegmentoDetalheFebraban240 item in lote.SegmentosDetalhe)
						{
							if (item.CodigoSegmentoRegistroDetalhe.Valor == EnumTipoSegmentoDetalheFebraban240.SEGMENTO_T.Codigo())
							{
								...
								ocorrenciaCobranca.Nossonumero = (item as SegmentoDetalheTFebraban240Padrao).IdentificacaoTituloNoBanco.Valor.Trim().Substring(2, 8);
