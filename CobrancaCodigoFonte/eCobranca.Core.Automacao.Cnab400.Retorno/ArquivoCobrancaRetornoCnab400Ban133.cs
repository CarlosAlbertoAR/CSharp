...
public class ArquivoCobrancaRetornoCnab400Ban133 : ArquivoCobrancaRetornoCnab400
{
	public ArquivoCobrancaRetornoCnab400Ban133(Guid guid, EnumTipoCarteiraCobranca tipoCarteiraCobranca)
		: base(guid, tipoCarteiraCobranca)
	{
		Logger.Debug($"Criando arquivo de cobrança de retorno CNAB400 do [{EnumCodigoBanco.CRESOL}]...");
		header = new HeaderRetornoCnab400Ban133();
		trailler = new TraillerRetornoCnab400Ban133();
	}
	...
	public override ArrayList CarregarArquivoRetornoCnab400()
	{
		...
		try
		{
			...
			{
				Logger.Debug($"Carregando arquivo de retorno CNAB400 do [{EnumCodigoBanco.CRESOL}]...");
				OcorrenciaCobranca ocorrenciaCobranca = null;
				if (base.Lote != null)
				{
					foreach (RegistroDetalheRetornoCnab400Ban133 item in base.Lote.SegmentosDetalhe)
					{
						ocorrenciaCobranca = new OcorrenciaCobranca();
						ocorrenciaCobranca.NumeroSequencialRetorno = int.Parse((header as HeaderRetornoCnab400Ban133).NumeroAvisoBancario.Valor.Trim());
						ocorrenciaCobranca.CodigoMovimentoRetorno = item.CodigoOcorrencia.Valor.Trim();
						ocorrenciaCobranca.DescricaoMovimentoRetorno = RepositorioDetalhesOcorrencia.GetInstance().GetDescricaoOcorrencia(EnumChaveContexto.BAN133_CNAB400, ocorrenciaCobranca.CodigoMovimentoRetorno);
						...
						ocorrenciaCobranca.CodigoBanco = (header as HeaderRetornoCnab400Ban133).CodigoBanco.Valor.Substring(0, 3);
	...
	private ArrayList CarregarMotivosOcorrencia(OcorrenciaCobranca ocorrencia, string motivos)
	{
		...
		{
			MotivoOcorrenciaCobranca motivoOcorrencia = RepositorioDetalhesOcorrencia.GetInstance().GetMotivoOcorrencia(EnumChaveContexto.BAN133_CNAB400, ocorrencia.CodigoMovimentoRetorno, motivos.Substring(0, 2));
