...
public class ArquivoCobrancaRetornoCnab400Ban084 : ArquivoCobrancaRetornoCnab400
{
	public ArquivoCobrancaRetornoCnab400Ban084(Guid guid, EnumTipoCarteiraCobranca tipoCarteiraCobranca)
		: base(guid, tipoCarteiraCobranca)
	{
		...
		header = new HeaderRetornoCnab400Ban084();
		trailler = new TraillerRetornoCnab400Ban084();
	}
	...
	public override ArrayList CarregarArquivoRetornoCnab400()
	{
		...
		try
		{
			...
			{
				Logger.Debug($"Carregando arquivo de retorno CNAB400 do [{EnumCodigoBanco.UNIPRIME_NORTE_PARANA}]...");
				OcorrenciaCobranca ocorrenciaCobranca = null;
				if (base.Lote != null)
				{
					foreach (RegistroTransacaoTipo1RetornoCnab400Ban084 item in base.Lote.SegmentosDetalhe)
					{
						ocorrenciaCobranca = new OcorrenciaCobranca();
						ocorrenciaCobranca.NumeroSequencialRetorno = int.Parse((header as HeaderRetornoCnab400Ban084).NumeroAvisoBancario.Valor.Trim());
						ocorrenciaCobranca.CodigoMovimentoRetorno = item.CodigoOcorrencia.Valor.Trim();
						ocorrenciaCobranca.DescricaoMovimentoRetorno = RepositorioDetalhesOcorrencia.GetInstance().GetDescricaoOcorrencia(EnumChaveContexto.BAN084_CNAB400, ocorrenciaCobranca.CodigoMovimentoRetorno);
						...
						ocorrenciaCobranca.CodigoBanco = (header as HeaderRetornoCnab400Ban084).CodigoBanco.Valor.Substring(0, 3);
