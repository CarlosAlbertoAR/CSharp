...
public class ComandoGerarSegmentoQRemessaFebraban240Ban237Unicred : ComandoGerarSegmentoQRemessaFebraban240Ban237
{
	public override void Execute()
	{
		try
		{
			Logger.Debug($"Executando o comando para gerar o SEGMENTO Q da remessa FEBRABAN240 do [{EnumCodigoBanco.BANCO_BRADESCO}]...");
			(base.Segmento as SegmentoDetalheQFebraban240Padrao).CodigoBancoCompensacao.Valor = EnumCodigoBanco.BANCO_BRADESCO.Codigo();
			...
			if (!string.IsNullOrEmpty(base.Carteira.ConfiguracaoCarteiraCobranca.NumeroInscricaoBeneficiario))
			{
				(base.Segmento as SegmentoDetalheQFebraban240Padrao).NumeroInscricaoSacadorAvalista.Valor = StringUtil.RemoverMascara(base.Carteira.ConfiguracaoCarteiraCobranca.NumeroInscricaoBeneficiario);
			}
			(base.Segmento as SegmentoDetalheQFebraban240Padrao).NomeSacadorAvalista.Valor = base.Carteira.ConfiguracaoCarteiraCobranca.NomeBeneficiario;
		...
		catch (Exception ex)
		{
			...
			{
				Launcher.GetInstance().LancarExcecao(ExcecaoECobranca.ModeloBoleto.FalhaExecutandoComandoGerarSegmentoQRemessaFebraban240(EnumCodigoBanco.BANCO_BRADESCO, ex), new StackTrace().GetFrame(1).GetMethod());
			}
		}
	}
}
