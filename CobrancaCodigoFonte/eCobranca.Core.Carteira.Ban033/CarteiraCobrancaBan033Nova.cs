...
[Serializable]
public abstract class CarteiraCobrancaBan033Nova : CarteiraCobrancaBan033, ISerializable
{
	public CarteiraCobrancaBan033Nova(ConfiguracaoCarteiraCobranca configuracaoCarteiraCobranca)
		: base(configuracaoCarteiraCobranca)
	{
	}

	public CarteiraCobrancaBan033Nova(SerializationInfo serobj, StreamingContext scontext)
	...
	protected override string GetLinhaDigitavel(DocumentoCobranca dc)
	{
		...
		try
		{
			...
			string text2 = EnumCodigoBanco.BANCO_SANTANDER.Codigo() + "99" + StringUtil.RemoverMascara((base.DadosConfiguracao[2] as DadoConfiguracao).Valor).Substring(0, 4);
			string text3 = text2 + DVGenerator.GetDVModulo10(text2);
			text3 = text3.Insert(5, ".");
			string text4 = StringUtil.RemoverMascara((base.DadosConfiguracao[2] as DadoConfiguracao).Valor).Substring(4, 3) + StringUtil.RemoverMascara(text).Substring(0, 7);
	...
	protected override string GetCampoLivre(DocumentoCobranca dc)
	{
		...
		try
		{
			...
			string value = "9";
			string value2 = StringUtil.RemoverMascara((base.DadosConfiguracao[2] as DadoConfiguracao).Valor).Substring(0, 7);
			string text = StringUtil.RemoverMascara(dc.NossoNumero);
			if (text.Length == 8)
			{
				text = "00000" + text;
