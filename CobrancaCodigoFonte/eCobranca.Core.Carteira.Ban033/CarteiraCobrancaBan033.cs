...
using eCobranca.Core.Carteira.Ban033.Comandos.Cnab400.Remessa;
using eCobranca.Core.Carteira.Ban033.Comandos.Cnab400.Retorno;
using eCobranca.Core.Carteira.Ban033.Comandos.Febraban240.Remessa;
using eCobranca.Core.Carteira.Ban033.Comandos.Febraban240.Retorno;
...
[Serializable]
public abstract class CarteiraCobrancaBan033 : CarteiraCobranca, ISerializable
{
	public CarteiraCobrancaBan033(ConfiguracaoCarteiraCobranca configuracaoCarteiraCobranca)
		: base(configuracaoCarteiraCobranca)
	{
	}

	public CarteiraCobrancaBan033(SerializationInfo serobj, StreamingContext scontext)
	...
	{
		return new FactorySegmentoFebraban240Ban033();
	}
	...
	protected override ConfiguracaoBoleto CarregarConfiguracaoBoleto()
	{
		...
		try
		{
			Logger.Debug($"Criando configuração inicial de boleto do [{EnumCodigoBanco.BANCO_SANTANDER}]...");
			...
			result = new ConfiguracaoBoleto(GetGuid(), EnumCodigoBanco.BANCO_SANTANDER.ToString(), base.ModoUso, this, "SAC Santander: 0800 762 7777 (Informações, elogios e reclamações) atende também deficientes auditivos ou de fala. Ouvidoria Santander: 0800 726 0322 (dias úteis, das 9h às 18h)", "033-7", GetCodigoCarteiraImpressao());
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override ConfiguracaoArquivoRemessa CarregarConfiguracaoArquivoRemessa()
	{
		...
		try
		{
			Logger.Debug($"Criando configuração inicial de remessa de carteiras do banco do [{EnumCodigoBanco.BANCO_SANTANDER}]");
			configuracaoArquivoRemessa = new ConfiguracaoArquivoRemessa(GetGuid(), EnumCodigoBanco.BANCO_SANTANDER.ToString(), base.ModoUso, this, 10, base.ConfiguracaoCarteiraCobranca.CobrancaRegistrada);
	...
	{
		return (ArquivoCobrancaRemessaFebraban240)FactoryArquivoCobranca.CriarArquivoRemessa(GetGuid(), EnumLayoutArquivoRemessa.FEBRABAN240, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.BANCO_SANTANDER);
	}

	protected override ArquivoCobrancaRemessaCnab400 CarregarArquivoRemessaCnab400()
	{
		return (ArquivoCobrancaRemessaCnab400Ban033)FactoryArquivoCobranca.CriarArquivoRemessa(GetGuid(), EnumLayoutArquivoRemessa.CNAB400, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.BANCO_SANTANDER);
	}

	protected override MacroCommand CarregarComandoGerarRemessaFebraban240()
	{
		return new ComandoGerarArquivoRemessaFebraban240Ban033(this);
	}

	protected override MacroCommand CarregarComandoGerarRemessaCnab400()
	{
		return new ComandoGerarArquivoRemessaCnab400Ban033();
	}
	...
	protected override ConfiguracaoArquivoRetorno CarregarConfiguracaoArquivoRetorno()
	{
		...
		try
		{
			Logger.Debug($"Criando configuração inicial de retorno de carteiras do banco do [{EnumCodigoBanco.BANCO_SANTANDER}]");
			result = new ConfiguracaoArquivoRetorno(GetGuid(), EnumCodigoBanco.BANCO_SANTANDER.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	{
		return (ArquivoCobrancaRetornoFebraban240)FactoryArquivoCobranca.CriarArquivoRetorno(GetGuid(), EnumLayoutArquivoRetorno.FEBRABAN240, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.BANCO_SANTANDER);
	}

	protected override ArquivoCobrancaRetornoCnab400 CarregarArquivoRetornoCnab400()
	{
		return (ArquivoCobrancaRetornoCnab400Ban033)FactoryArquivoCobranca.CriarArquivoRetorno(GetGuid(), EnumLayoutArquivoRetorno.CNAB400, (EnumTipoCarteiraCobranca)(object)base.ConfiguracaoCarteiraCobranca.TipoCarteiraCobranca.ToEnum<EnumTipoCarteiraCobranca>(), EnumCodigoBanco.BANCO_SANTANDER);
	}
	...
	{
		return new ComandoCarregarArquivoRetornoFebraban240Ban033(this);
	}

	protected override MacroCommand CarregarComandoProcessarRetornoCnab400()
	{
		return new ComandoCarregarArquivoRetornoCnab400Ban033();
	}
	...
	protected override ConfiguracaoImpressao CarregarConfiguracaoImpressao()
	{
		...
		try
		{
			Logger.Debug($"Carregando configuração de impressão de documentos de cobrança do [{EnumCodigoBanco.BANCO_SANTANDER}]...");
			result = new ConfiguracaoImpressao(GetGuid(), EnumCodigoBanco.BANCO_SANTANDER.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override ConfiguracaoEmail CarregarConfiguracaoEmail()
	{
		...
		{
			Logger.Debug($"Carregando configuração de impressão de documentos de cobrança do [{EnumCodigoBanco.BANCO_SANTANDER}]...");
			result = new ConfiguracaoEmail(GetGuid(), EnumCodigoBanco.BANCO_SANTANDER.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override ConfiguracaoDocumentoCobranca CarregarConfiguracaoDocumentoCobranca()
	{
		...
		try
		{
			Logger.Debug($"Carregando configuração de documentos de cobrança do [{EnumCodigoBanco.BANCO_SANTANDER}]...");
			result = new ConfiguracaoDocumentoCobrancaBan033(GetGuid(), EnumCodigoBanco.BANCO_SANTANDER.ToString(), base.ModoUso, this);
			return result;
		}
		catch (Exception ex)
		{
	...
	protected override string GerarArquivoRemessaFebraban240(ArrayList dcList)
	{
		...
		try
		{
			Logger.Debug($"Gerando arquivo de remessa FEBRABAN240 para o [{EnumCodigoBanco.BANCO_SANTANDER}]...");
			remessaFebraban240.Preparar(this);
			remessaFebraban240.NovoLote(dcList);
			(comandoGerarRemessaFebraban240 as ComandoGerarArquivoRemessaFebraban240Ban033).Preparar(this, remessaFebraban240);
	...
	protected override string GerarArquivoRemessaCNAB400(ArrayList dcList)
	{
		...
		try
		{
			...
			{
				Logger.Debug($"Gerando arquivo de remessa CNAB400 para o [{EnumCodigoBanco.BANCO_SANTANDER}]...");
				remessaCnab400.Preparar(base.ConfiguracaoArquivoRemessa);
				remessaCnab400.CriarLote(dcList);
				(comandoGerarRemessaCnab400 as ComandoGerarArquivoRemessaCnab400Ban033).Preparar(this, remessaCnab400);
	...
	protected override ArrayList CarregarArquivoRetornoFebraban240(string nomeArquivoRetorno)
	{
		...
		try
		{
			Logger.Debug($"Processando arquivo de retorno FEBRABAN240 do [{EnumCodigoBanco.BANCO_SANTANDER}]...)");
			retornoFebraban240.Preparar(this, base.ConfiguracaoArquivoRetorno.DiretorioArquivoRetorno, nomeArquivoRetorno);
			(comandoProcessarRetornoFebraban240 as ComandoCarregarArquivoRetornoFebraban240Ban033).Preparar(this, retornoFebraban240);
	...
	protected override ArrayList CarregarArquivoRetornoCnab400(string nomeArquivoRetorno)
	{
		...
		try
		{
			Logger.Debug($"Processando arquivo de retorno CNAB400 do [{EnumCodigoBanco.BANCO_SANTANDER}]...");
			retornoCnab400.Preparar(base.ConfiguracaoArquivoRetorno.DiretorioArquivoRetorno, nomeArquivoRetorno);
			(comandoProcessarRetornoCnab400 as ComandoCarregarArquivoRetornoCnab400Ban033).Preparar(this, retornoCnab400);
	...
	{
		return new ProvedorDadosModeloBoletoBan033();
	}
	...
	protected override string GetNossoNumeroSemDV(DocumentoCobranca dc)
	{
		...
		try
		{
			...
			string mascara = (base.DadosConfiguracao[5] as DadoConfiguracao).Mascara;
			string valor = (base.DadosConfiguracao[5] as DadoConfiguracao).Valor;
			string valor2 = (base.DadosConfiguracao[6] as DadoConfiguracao).Valor;
			string valor3 = (base.DadosConfiguracao[7] as DadoConfiguracao).Valor;
			...
			(base.DadosConfiguracao[7] as DadoConfiguracao).Valor = valor3;
			result = valor3;
			return result;
		}
		catch (Exception ex)
	...
	protected override string CalcularDVNossoNumero(string nossoNumeroSemDV)
	{
		...
		try
		{
			string value = StringUtil.RemoverMascara(nossoNumeroSemDV);
	...
	{
		return new ParametrosAdicionaisDisponiveisBan033();
	}
